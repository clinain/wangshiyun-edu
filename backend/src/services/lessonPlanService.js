...
const { getSubjectConfig, getStageByGrade, subjectsConfig, getDimensionConfig } = require('../config/subjectsConfig');
...
## 教案JSON结构
{
  ...
  "teachingGoals": {
    ...
    "dimensions": [
      {
        "id": "核心素养维度ID（如language_construction）",
        "name": "核心素养维度名称（如语言建构与运用）",
        "goals": [
          "该维度下的具体教学目标1",
          "该维度下的具体教学目标2"
        ]
...
## 教学目标重要说明
- teachingGoals必须包含version字段，值为2
...
const generateSubjectAwarePrompt = (topic, grade, subject, additionalRequirements = {}) => {
  ...
  // 获取学科核心素养维度配置
  const dimensionConfig = getDimensionConfig(subject, stage);
...
const generateLessonPlan = async (topic, grade, subject, options = {}) => {
  try {
    ...
    const assistantMessage = await AIService.callAPI(systemPrompt, prompt, {
      provider: options.provider || options.aiProvider,
    ...
    const aiInfo = AIService.getServiceInfo();
    ...
    return {
      ...
      data: {
        ...
        metadata: {
          ...
          model: aiInfo.currentProviderInfo?.model || aiInfo.currentProvider
        }
      }
    };
  } catch (error) {
...
const validateLessonPlan = (lessonPlan) => {
  const required = ['basicInfo', 'teachingGoals', 'teachingKeyPoints', 'teachingProcess'];
